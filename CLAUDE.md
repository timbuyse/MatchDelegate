...
Test-data scripts live in Tim's companion folder outside this repo, never committed here. They are self-contained IIFEs pasted into the browser console. **With an admin signed in, `cloudReady` is true and any save reaches production data**, so which rules apply depends on who runs the script:
